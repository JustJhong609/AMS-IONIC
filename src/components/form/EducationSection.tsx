...
import {
  CURRENTLY_STUDYING_OPTIONS, GRADE_LEVELS, REASON_OPTIONS,
  YES_NO_OPTIONS, OCCUPATION_TYPE_OPTIONS, EMPLOYMENT_STATUS_OPTIONS,
...
const EducationSection: React.FC<Props> = ({ data, errors, onChange }) => (
  <div>
    ...
    <RadioGroup label="Basic Literacy Program (BLP)?" options={['Yes', 'No']}
      value={data.isBlp} onChange={v => onChange('isBlp', v)} />

    <FormSelect label="Occupation Type (optional)" value={data.occupationType}
      onChange={v => onChange('occupationType', v)} options={OCCUPATION_TYPE_OPTIONS} />

    <FormSelect label="Employment Status (optional)" value={data.employmentStatus}
      onChange={v => onChange('employmentStatus', v)} options={EMPLOYMENT_STATUS_OPTIONS} />

    <FormInput label="Monthly Income (optional)" value={data.monthlyIncome}
      onChange={v => onChange('monthlyIncome', v)} type="number" inputmode="numeric"
      placeholder="e.g. 5000" />

    <RadioGroup label="Interested in ALS? *" options={YES_NO_OPTIONS as unknown as string[]}
      value={data.interestedInALS} onChange={v => onChange('interestedInALS', v)}
      error={errors.interestedInALS} />

    <FormInput label="Contact Number (optional)" value={data.contactNumber}
